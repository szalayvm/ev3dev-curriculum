...
def main():
    ...
    forward_button['command'] = lambda: handle_forward(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get()))
    root.bind('<Up>', lambda event: handle_forward(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get())))

    left_button = ttk.Button(main_frame, text="Left")
    left_button.grid(row=3, column=0)
    # left_button and '<Left>' key
    left_button['command'] = lambda: handle_left(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get()))
    root.bind('<Left>', lambda event: handle_left(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get())))
    ...
    right_button['command'] = lambda: handle_right(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry).get())
    root.bind('<Right>', lambda event: handle_right(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get())))

    back_button = ttk.Button(main_frame, text="Back")
    back_button.grid(row=4, column=1)
    # back_button and '<Down>' key
    back_button['command'] = lambda: handle_back(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry).get())
    root.bind('<Down>', lambda event: handle_back(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get())))
...
def handle_left(mqtt_client, left_speed_entry, right_speed_entry):
    mqtt_client.send_message('drive_forever', [-1*left_speed_entry, right_speed_entry])

def handle_right(mqtt_client, left_speed, right_speed):
    mqtt_client.send_message('drive_forever', [left_speed, -1*right_speed])

def handle_forward(mqtt_client, left_speed, right_speed):
    mqtt_client.send_message('drive_forever', [left_speed, right_speed])

def handle_back(mqtt_client, left_speed, right_speed):
    mqtt_client.send_message('drive_forever', [-1*left_speed, -1*right_speed])
